...
const TrashListBox = () => {
  ...
  const filteredDocuments = resumes?.filter((doc: TrashDocument) => {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        {/* Replace button with div to avoid nesting buttons */}
        <div
          role="button"
          tabIndex={0}
          className="inline-flex items-center text-[15px] gap-[2px] px-3 py-2 border border-input bg-background hover:bg-accent hover:text-accent-foreground rounded-md text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50"
        >
          <Trash2 size="15px" />
          <span>All Trash</span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-sm">
            ...
            "
              ...
              {filteredDocuments?.map((doc: TrashDocument) => (
                <div
                  key={doc.id}
                  role="button"
                  onClick={() => onClick(doc.documentId)}
                  className="text-sm rounded-s w-full hover:bg-primary/5
